...
class RobustBVBRCHandler:
    ...
    def load_representative_genomes(self, limit: Optional[int] = None) -> Dict[str, Dict]:
        """Load biomarker genomes from combo_top_biomarkers.tsv"""
        
        biomarker_file = 'combo_top_biomarkers.tsv'
        biomarker_genomes = {}
        ...
        try:
            with open(biomarker_file, 'r') as f:
                ...
                for line in lines[1:]:  # Skip header
                    ...
                    if len(parts) >= 2 and parts[0] and parts[1]:  # Valid data row
                        biomarker_name = parts[0].strip()
                        genome_id = parts[1].strip()
                        
                        biomarker_genomes[genome_id] = {
                            'genome_name': f"Biomarker_{biomarker_name}",
            ...
            print(f"✅ Loaded {len(biomarker_genomes)} biomarker genomes from combo_top_biomarkers.tsv")
            return biomarker_genomes
            
        except Exception as e:
            print(f"❌ Error loading biomarker genomes: {e}")
            return {}
    ...
    def search_gene_in_genome(self, gene_term: str, genome_id: str, search_type: str = 'gene') -> Dict:
        ...
        if search_type == 'gene':
            # Gene name search - try both exact match and keyword search
            query = f'and(eq(genome_id,{genome_id}),or(eq(gene,"{gene_term}"),keyword("{gene_term}")))'
        ...
        if success:
            if len(data) > 0:
                print(f"    SUCCESS: Found {len(data)} features. First result genome_id: {data[0].get('genome_id', 'MISSING')}")
            return {
                "success": True,
                "genome_id": genome_id,
                "gene_term": gene_term,
